...
ERP (via CDC)     →  writes  suppliers, purchase_orders
...
**MongoDB as ODL is well-suited to this system for three reasons:**
...
**ODL ownership rule** — only the owning service writes to a collection. This means schema changes are always initiated by the owner, never by a consumer. The owner is responsible for backward compatibility or coordinating the migration.
...
**`suppliers`** — CN/TW suppliers carry `tariff_exposure_rating`. Fresh produce suppliers carry `cold_chain_certified`. Audited suppliers carry `sustainability_verified`. Each document carries only the fields that are real for that supplier. Consumers check field presence before using it — no null columns.
...
Atlas Charts (the dashboard) reads from the same ODL in real time. There is no separate reporting database.
